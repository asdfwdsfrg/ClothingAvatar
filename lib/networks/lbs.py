...
def lbs(betas, pose, v, shapedirs, posedirs, J, parents,
        lbs_weights, pose2rot=True, new_params=False, dtype=torch.float32, inverse = False):
    ...
    T = torch.matmul(W, A.contiguous().view(batch_size, num_joints, 16)) \
        .contiguous().view(batch_size, -1, 4, 4)
    homogen_shape = list(v_posed.shape)
    homogen_shape[-1] = 1
